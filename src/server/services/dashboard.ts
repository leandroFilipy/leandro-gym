...
import { getForgottenMuscles, getStagnationAlerts, getWeeklyMuscleVolume } from "./insights";
...
export async function getDashboard(userId: string) {
  ...
  const [user, todayInfo, week, lastSession, totals, goal, weight, records, stagnation, muscleVolume, water, forgotten] = await Promise.all([
    ...
    getForgottenMuscles(userId),
  ]);

  const greeting = hour < 12 ? "Bom dia" : hour < 18 ? "Boa tarde" : "Boa noite";
  const firstName = user?.name?.split(" ")[0] ?? null;

  return { greeting, firstName, today, todayInfo, week, lastSession, totals, goal, weight, records, stagnation, muscleVolume, water, forgotten };
}
